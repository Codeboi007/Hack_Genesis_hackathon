...
class RepoInput(BaseModel):
    ...
    persona: Persona
    encrypted_docs_token: str | None = Field(
        default=None,
        description="Encrypted GitHub PAT for docs push, generated by backend verify endpoint.",
    )
    docs_token: str | None = Field(
        default=None,
        description="Raw GitHub PAT for one-time docs push fallback when verify endpoint is unavailable.",
    )


class TokenVerifyInput(BaseModel):
    repo_url: str = Field(..., description="GitHub repository URL to validate token access against")
    token: str = Field(..., min_length=20, description="Fine-grained GitHub PAT")
